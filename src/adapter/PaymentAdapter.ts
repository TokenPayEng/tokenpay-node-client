...
export default class PaymentAdapter extends BaseAdapter {
  ...
  async initWalletPayment(request: any) {
    return this._client.post('/payment/v1/wallet/init', request);
  }

  async inquireWalletPayment(orderId: string): Promise<PaymentResponse> {
    return this._client.get('/payment/v1/wallet/status/' + orderId);
  }
}
