...
test('listBouncedSubMerchantRows() should search bounced settlement rows for sub merchants', async t => {
  ...
  const {page, size, totalSize, items} = await settlementReportingAdapter.searchBouncedSubMerchantRows(request);
  t.is(page, 0);
  t.is(size, 25);
  t.is(totalSize, 2);
  t.is(items.length, 2);
...
test('listPayoutCompletedTransactions() should search settlement rows for transactions that are already paid out', async t => {
  const startDate = new Date(Date.parse('2020-09-11 08:00:00+03:00'));
  const endDate = new Date(Date.parse('2020-09-12 08:00:00+03:00'));
  ...
  const {page, size, totalSize, items} = await settlementReportingAdapter.searchPayoutCompletedTransactions(request);
  t.is(page, 0);
  t.is(size, 25);
  t.is(totalSize, 1);
  t.is(items.length, 1);
