...
test('HttpClient should translate API errors to TokenPayError errors when status code is 200', async t => {
  ...
  t.true(caughtException instanceof TokenPayError);
...
test('HttpClient should translate API errors to TokenPayError errors when status code is not 200', async t => {
  const client = new HttpClient({
    apiKey: 'dummy',
    secretKey: 'dummy',
    baseUrl: 'http://localhost:8000'
